...
impl Blake2bHasher {
  ...
  pub fn init(&mut self) -> Result<(), ErrorCode> {
    ...
    let rc = unsafe {
      cx_hash_init_ex(
        self.0.as_mut_ptr(),
        CX_BLAKE2B,
        BLAKE2B_HASH_SIZE as u32,
      )
    };
    if rc == CX_OK { Ok(()) } else { Err(ErrorCode::TxSignFail) }
  }

  pub fn update(&mut self, input: &[u8]) -> Result<(), ErrorCode> {
    let rc = unsafe { cx_hash_update(self.0.as_mut_ptr(), input.as_ptr(), input.len() as c_uint) };
    if rc == CX_OK { Ok(()) } else { Err(ErrorCode::TxSignFail) }
  }
  ...
  pub fn finalize(&mut self) -> Result<[u8; BLAKE2B_HASH_SIZE], ErrorCode> {
    ...
    if rc == CX_OK { Ok(result) } else { Err(ErrorCode::TxSignFail) }
  }
}
