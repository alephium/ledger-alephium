...
mod tests {
    ...
    use super::{PartialDecoder, UnsignedTx};
    use crate::buffer::Buffer;
    ...
    use crate::types::{Hash, Hint, LockupScript, PublicKey, UnlockScript, I32, U256};
    ...
    impl TxDecodeContext {
        pub fn decode<'a>(
            &mut self,
            buffer: &mut Buffer<'a>,
        ) -> Result<Option<&UnsignedTx>, DecodeError> {
    ...
    fn test_decode_transfer_alph_tx() {
        ...
        let check = |expected: &UnsignedTx| {
            ...
            let output_ref_key_bytes =
                hex_to_bytes("950bf46c8d7fe6ca54a2cffdbc29f60c9b666fb42cb1c09a17d2ff555e3e893e")
                    .unwrap();
            let public_key_bytes =
                hex_to_bytes("02622da4723abe3e57e6926b69a049635dad0f9059a89ca222d83f0b2da256235e")
                    .unwrap();
            ...
            let public_key_hash_bytes =
                hex_to_bytes("9b85f066b1b2821339bf73e9e00bbe660b0cfb97158ceedff3260e1e4368961d")
                    .unwrap();
    ...
    fn test_decode_transfer_token_tx() {
        ...
        let check = |expected: &UnsignedTx| {
            ...
            let output_ref_key_bytes =
                hex_to_bytes("3cfed394414a0238ab8be798b88140c4f9255f094f30614f184afa0ba5984ba0")
                    .unwrap();
            let public_key_bytes =
                hex_to_bytes("02e835a6e954a0a0b0e540f4451186e5a1f99baf93a111d304866945a768c39d5c")
                    .unwrap();
            assert_eq!(last_input.hint, Hint::from_bytes(input_hint_bytes));
            assert_eq!(
                last_input.key,
            ...
            let public_key_hash_bytes =
                hex_to_bytes("4e796b6f3b889eb8959c285ea4ef8dea6d7aad4c444e2f83f3403fdfde5d2eb6")
                    .unwrap();
            assert_eq!(u256_to_string(&last_output.amount), atto_alph_amount);
            assert_eq!(
                last_output.lockup_script,
